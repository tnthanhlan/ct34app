...
app.post('/api/state/grid-clear-month', requireAuth, requireAdmin, (req, res) => {
  ...
  const nDays = daysInMonth(y, m);
  let count = 0;
  db.state.employees.forEach(emp => {
...
/* ---------------- (Da go bo: tinh nang "chot bu 12 thang" tu dong) ----------------
   Ly do go bo: no dung CAU HINH DANG CO TAI THOI DIEM KHOI DONG SERVER de tinh mã cho ca
   nhung thang da qua - neu admin da doi Kip/lich (du dinh ap dung cho thang SAU) TRUOC KHI
   restart/update, no se "chot" nham gia tri MOI (sai) de vao ca nhung thang CU (dung), pha
   huy du lieu lich su dung. Day la loi thiet ke nghiem trong da xay ra that (07-08/2026).
   Tu gio, viec chot thang CHI xay ra qua 2 con duong CHU DONG, KHONG tu dong bat ngo:
   1) Nut "Chot du lieu thang nay" - admin tu bam dung luc con dung du lieu.
   2) Cron dung 00:05 ngay 1 hang thang - chi chot DUNG 1 thang vua ket thuc, tai dung
      thoi diem co dinh, KHONG chay lai moi lan restart va KHONG quet nguoc nhieu thang. */
