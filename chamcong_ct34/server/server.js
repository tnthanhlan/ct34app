...
(function loadHaOptions(){
  ...
  try{
    ...
    if (opts.onedrive_backup_enabled) process.env.ONEDRIVE_BACKUP_ENABLED = 'true';
    if (opts.onedrive_token) process.env.ONEDRIVE_TOKEN = opts.onedrive_token;
    if (opts.onedrive_remote_path) process.env.ONEDRIVE_REMOTE_PATH = opts.onedrive_remote_path;
    if (opts.onedrive_drive_id) process.env.ONEDRIVE_DRIVE_ID = opts.onedrive_drive_id;
    if (opts.onedrive_drive_type) process.env.ONEDRIVE_DRIVE_TYPE = opts.onedrive_drive_type;
  }catch(e){
...
/* ---------------- Tu dong day file xuat len OneDrive (dung rclone) ----------------
   Muc dich: thay the hoan toan cach cu (may Windows dung Task Scheduler + Samba keo file ve -
   chi chay khi may do cung mang LAN voi Home Assistant). Cach nay HA tu day thang len OneDrive
   qua Internet ngay khi vua xuat file xong, khong phu thuoc may Windows nao ca, khong phu thuoc mang LAN.

   Cach lay ONEDRIVE_TOKEN (lam 1 lan duy nhat, tren may co trinh duyet - vi du chinh may Windows):
   1. Cai rclone tren may do: https://rclone.org/downloads/
   2. Mo PowerShell/CMD, chay:  rclone authorize "onedrive"
   3. Trinh duyet se mo ra, dang nhap dung tai khoan Microsoft dang chua thu muc Shared can day len.
   4. Sau khi dang nhap xong, terminal se in ra 1 doan JSON (bat dau bang {"access_token":...).
      Copy NGUYEN VAN doan JSON do (ca dau { va cuoi }), dan vao o "onedrive_token" trong tab
      Configuration cua add-on nay.
   5. Dien "onedrive_remote_path" = duong dan (tinh tu goc OneDrive) toi dung thu muc Shared do.
      Kiem tra truoc bang: rclone lsd onedrive: (roi lsd tiep vao tung cap) de biet chinh xac ten. */
...
const ONEDRIVE_LOG = []; // vai dong log gan nhat, hien trong Admin de de kiem tra khong can SSH
...
function syncFileToOnedrive(localFilePath) {
  ...
  execFile('rclone', args, { timeout: 120000 }, (err, stdout, stderr) => {
    if (err) {
      onedriveLog(`LỖI khi đồng bộ "${path.basename(localFilePath)}": ${(stderr || err.message).toString().split('\n')[0]}`);
    } else {
...
async function autoExportMonth(y, m) {
  ...
  try {
    ...
    const fpathMeal = path.join(EXPORT_DIR, fnameMeal);
    await wbMeal.xlsx.writeFile(fpathMeal);
    console.log(`[auto-export] Đã lưu ${fname} + ${fnameMeal} vào ${EXPORT_DIR}`);
    syncFileToOnedrive(fpath);
    syncFileToOnedrive(fpathMeal);
  } catch (e) {
...
app.get('/api/admin/onedrive-status', requireAuth, requireAdmin, (req, res) => {
  res.json({
...
app.post('/api/admin/onedrive-sync-now', requireAuth, requireAdmin, (req, res) => {
  if (!onedriveReady) {
    return res.status(400).json({ error: 'Chưa bật/cấu hình xong đồng bộ OneDrive (xem tab Configuration của add-on).' });
  }
  try {
    ensureExportDir();
    const files = fs.readdirSync(EXPORT_DIR).filter(f => /\.xlsx$/i.test(f));
    files.forEach(f => syncFileToOnedrive(path.join(EXPORT_DIR, f)));
    onedriveLog(`Đồng bộ thủ công: đã gửi lệnh cho ${files.length} file.`);
