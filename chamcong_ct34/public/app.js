...
async function renderOnedriveStatus(){
  const line = document.getElementById('onedriveStatusLine');
  const body = document.getElementById('onedriveLogBody');
  line.textContent = 'Đang tải trạng thái...';
  try{
    const st = await api('GET', '/api/admin/onedrive-status');
    if(!st.enabled){
      line.textContent = 'Chưa bật đồng bộ OneDrive (bật "onedrive_backup_enabled" trong Configuration của add-on).';
    } else if(!st.ready){
      line.textContent = 'Đã bật nhưng chưa cấu hình xong (thiếu onedrive_token hoặc onedrive_remote_path) — xem nhật ký bên dưới.';
    } else {
      line.textContent = 'Đang hoạt động — đích: ' + st.remotePath;
    }
    body.innerHTML = '';
    if(!st.log || !st.log.length){
      body.innerHTML = '<tr><td>Chưa có nhật ký nào.</td></tr>';
    } else {
      st.log.forEach(line=>{
        const tr = document.createElement('tr');
        const td = document.createElement('td'); td.textContent = line; tr.appendChild(td);
        body.appendChild(tr);
      });
    }
  }catch(e){
    line.textContent = 'Không tải được trạng thái: ' + e.message;
  }
}
document.getElementById('btnRefreshOnedrive').addEventListener('click', renderOnedriveStatus);
document.getElementById('btnOnedriveSyncNow').addEventListener('click', async ()=>{
  try{
    const r = await api('POST', '/api/admin/onedrive-sync-now', {});
    alert('Đã gửi lệnh đồng bộ cho ' + r.count + ' file. Xem nhật ký bên dưới sau vài giây.');
    setTimeout(renderOnedriveStatus, 3000);
  }catch(e){
    alert('Không đồng bộ được: ' + e.message);
  }
});
...
async function bootApp(){
  ...
  renderCalendar();
  renderDangKy();
  if(currentRole==='admin'){ renderSnapshots(); renderShareBackups(); renderOnedriveStatus(); }
}
