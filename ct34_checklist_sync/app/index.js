...
// (2026-09-25) onedrive.js (copy nguyên văn từ add-on Bảo trì) chỉ export syncFileToOnedrive
// (fire-and-forget, không đợi được) và syncFilesSequentially (đợi được, chạy tuần tự) -
// KHÔNG export syncFileToOnedrivePromise dù hàm đó có định nghĩa nội bộ trong file. Dùng
// syncFilesSequentially([filePath]) cho đúng hàm thực sự được export, để có thể await log kết quả.
const { syncFilesSequentially, getOnedriveStatus } = require('./src/onedrive');
...
async function runSyncOnce(reason) {
  ...
  try {
    ...
    await syncFilesSequentially([filePath]);
