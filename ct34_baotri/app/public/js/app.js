const state = { user: null, view: 'dashboard', engines: [], engineMap: {}, engineFields: [], logMap: {}, maintenanceCategories: [], materials: [], materialFields: [] };
...
async function loadMaterialFields(force = false) {
  if (!force && state.materialFields.length) return state.materialFields;
  const { items } = await api('/material-fields');
  state.materialFields = items;
  return items;
}

// Quản lý trường dữ liệu vật tư (thêm/sửa/xóa/sắp xếp) - mirror hệt Quản lý trường dữ liệu động cơ
function openMaterialFieldManager(onDone) {
  const fields = state.materialFields;
  const html = `
    <div class="modal-title">Quản lý trường dữ liệu vật tư</div>
    <p style="font-size:12.5px; color:var(--ink-dim); margin-top:-6px;">
      Đây là các cột thông tin cho từng vật tư (TSKT, Mã kho...) — tự thêm mới, đổi tên, xóa, hoặc
      sắp xếp lại, không cần nhờ ai sửa code.
    </p>
    <div id="material-field-manager-list" style="max-height:42vh; overflow-y:auto; margin:10px 0;">
      ${fields.length ? fields.map((f, i) => `
        <div class="list-item" data-id="${f.id}">
          <div class="list-item-main">
            <div class="list-item-title">${escapeHtml(f.label)}</div>
            <div class="list-item-sub">${escapeHtml(f.field_key)}</div>
          </div>
          <div style="display:flex; gap:4px; flex-shrink:0;">
            <button type="button" class="btn btn-ghost btn-sm" data-act="up" ${i === 0 ? 'disabled' : ''}>↑</button>
    ...
    <div style="display:flex; gap:8px;">
      <input type="text" id="new-material-field-label" placeholder="Tên trường mới, vd: Nhà cung cấp..." style="flex:1; padding:9px 10px; border:1.5px solid var(--border); border-radius:7px;">
      <button type="button" class="btn btn-primary" id="add-material-field-btn">+ Thêm</button>
    </div>
    <div class="modal-actions">
      <button type="button" class="btn btn-primary btn-block" id="done-material-field-manager-btn">Xong</button>
    </div>
  `;
  openModal(html);

  document.getElementById('done-material-field-manager-btn').addEventListener('click', () => { closeModal(); if (onDone) onDone(); });

  document.getElementById('add-material-field-btn').addEventListener('click', async () => {
    const input = document.getElementById('new-material-field-label');
    const label = input.value.trim();
    if (!label) return;
    try {
      await api('/material-fields', { method: 'POST', body: JSON.stringify({ label }) });
      await loadMaterialFields(true);
      closeModal(); openMaterialFieldManager(onDone);
    } catch (err) { toast(err.message); }
  });

  document.querySelectorAll('#material-field-manager-list .list-item').forEach(row => {
    const id = Number(row.dataset.id);
    row.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', async () => {
        const act = btn.dataset.act;
        const idx = state.materialFields.findIndex(f => f.id === id);
        try {
          if (act === 'up' || act === 'down') {
            const arr = [...state.materialFields];
            const swapIdx = act === 'up' ? idx - 1 : idx + 1;
            [arr[idx], arr[swapIdx]] = [arr[swapIdx], arr[idx]];
            await api('/material-fields/reorder', { method: 'POST', body: JSON.stringify({ orderedIds: arr.map(f => f.id) }) });
          } else if (act === 'rename') {
            const cur = state.materialFields[idx];
            const newLabel = prompt('Đổi tên trường:', cur.label);
            if (!newLabel || !newLabel.trim()) return;
            await api('/material-fields/' + id, { method: 'PUT', body: JSON.stringify({ label: newLabel.trim() }) });
          } else if (act === 'del') {
            if (!confirm('Xóa trường này? Dữ liệu đã nhập cho trường này ở tất cả vật tư sẽ không còn hiển thị nữa.')) return;
            await api('/material-fields/' + id, { method: 'DELETE' });
          }
          await loadMaterialFields(true);
          closeModal(); openMaterialFieldManager(onDone);
        } catch (err) { toast(err.message); }
...
function openMaterialManager(onDone) {
  ...
  const fields = state.materialFields;
  const subLine = (m) => {
    const parts = fields.filter(f => m[f.field_key]).map(f => `${f.label}: ${m[f.field_key]}`);
    return parts.length ? parts.join(' · ') : 'Chưa có thông tin thêm';
  };
  const html = `
    ...
    <button type="button" class="btn btn-ghost btn-sm" id="manage-material-fields-btn" style="margin-bottom:10px;">⚙ Quản lý trường dữ liệu vật tư (${fields.length} trường)</button>
      ...
      ${mats.length ? mats.map((m, i) => `
        <div class="list-item" data-id="${m.id}">
          <div class="list-item-main">
            ...
            <div class="list-item-sub">${escapeHtml(subLine(m))}</div>
          </div>
          <div style="display:flex; gap:4px; flex-shrink:0;">
            <button type="button" class="btn btn-ghost btn-sm" data-act="up" ${i === 0 ? 'disabled' : ''}>↑</button>
            <button type="button" class="btn btn-ghost btn-sm" data-act="down" ${i === mats.length - 1 ? 'disabled' : ''}>↓</button>
            <button type="button" class="btn btn-ghost btn-sm" data-act="edit">Sửa</button>
            <button type="button" class="btn btn-danger btn-sm" data-act="del">Xóa</button>
          </div>
        </div>
    ...
    <div class="modal-actions">
      <button type="button" class="btn btn-ghost btn-block" id="add-material-btn2">+ Thêm vật tư mới</button>
  ...
  document.getElementById('add-material-btn2').addEventListener('click', () => openMaterialForm(null, onDone));
  document.getElementById('manage-material-fields-btn').addEventListener('click', () => {
    closeModal();
    openMaterialFieldManager(() => openMaterialManager(onDone));
  });
  ...
  document.querySelectorAll('#material-manager-list .list-item').forEach(row => {
    ...
    row.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (act === 'edit') { openMaterialForm(state.materials[idx], onDone); return; }
        try {
          if (act === 'up' || act === 'down') {
...
// Form thêm/sửa 1 vật tư - các trường hiện ra tự động theo danh sách trường đã cấu hình (TSKT, Mã kho, hoặc bất kỳ trường nào admin tự thêm)
function openMaterialForm(material, onDone) {
  const fields = state.materialFields;
  const html = `
    <div class="modal-title">${material ? 'Sửa vật tư' : 'Thêm vật tư mới'}</div>
    <form id="material-form" class="form-grid">
      <div class="full"><label>Tên vật tư
        <input type="text" name="name" required value="${material ? escapeAttr(material.name) : ''}" placeholder="vd: Aptomat MCB 20A">
      </label></div>
      ${fields.map(f => `
        <div class="full"><label>${escapeHtml(f.label)}
          <input type="text" name="${escapeAttr(f.field_key)}" value="${material ? escapeAttr(material[f.field_key] || '') : ''}">
        </label></div>
      `).join('')}
      <div class="full" style="text-align:right; margin-top:-2px;">
        <button type="button" class="btn btn-ghost btn-sm" id="manage-material-fields-inline-btn">⚙ Quản lý trường dữ liệu</button>
      </div>
      <div class="modal-actions full">
        <button type="button" class="btn btn-ghost" id="cancel-material-btn">Hủy</button>
        <button type="submit" class="btn btn-primary">Lưu</button>
      </div>
    </form>
  `;
  openModal(html);
  document.getElementById('cancel-material-btn').addEventListener('click', () => { closeModal(); openMaterialManager(onDone); });
  document.getElementById('manage-material-fields-inline-btn').addEventListener('click', () => {
    closeModal();
    openMaterialFieldManager(() => openMaterialForm(material, onDone));
  });
  document.getElementById('material-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const body = Object.fromEntries(new FormData(e.target).entries());
    try {
      if (material) await api('/materials/' + material.id, { method: 'PUT', body: JSON.stringify(body) });
      else await api('/materials', { method: 'POST', body: JSON.stringify(body) });
      await loadMaterials(true);
      closeModal(); toast('Đã lưu'); openMaterialManager(onDone);
    } catch (err) { toast(err.message); }
  });
}
...
async function openMaintenanceStatsModal() {
  ...
  const html = `
    ...
    <p style="font-size:12px; color:var(--ink-dim); margin-top:0; margin-bottom:8px;">💡 Kéo cạnh phải mỗi cột (rê chuột tới sát viền cột) để co giãn độ rộng theo ý bạn.</p>
    <div id="stats-table-wrap" style="overflow:auto; max-height:58vh; border:1px solid var(--border); border-radius:8px;">
      ${items.length ? `
      <table id="stats-table" style="width:100%; border-collapse:collapse; font-size:13px; table-layout:fixed;">
        <colgroup>
          <col style="width:120px;"><col style="width:170px;"><col style="width:160px;">
          <col style="width:100px;"><col style="width:140px;"><col style="width:280px;">
        </colgroup>
        <thead>
          <tr style="border-bottom:2px solid var(--border);">
            <th style="text-align:left; padding:8px 10px 8px 7px; position:sticky; top:0; background:var(--surface); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Mã thiết bị</th>
            <th style="text-align:left; padding:8px 10px 8px 7px; position:sticky; top:0; background:var(--surface); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Tên gọi</th>
            <th style="text-align:left; padding:8px 10px 8px 7px; position:sticky; top:0; background:var(--surface); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Hạng mục</th>
            <th style="text-align:left; padding:8px 10px 8px 7px; position:sticky; top:0; background:var(--surface); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Ngày</th>
            <th style="text-align:left; padding:8px 10px 8px 7px; position:sticky; top:0; background:var(--surface); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Người thực hiện</th>
            <th style="text-align:left; padding:8px 7px; position:sticky; top:0; background:var(--surface);">Nội dung</th>
          </tr>
        </thead>
        <tbody>
          ${items.map(l => `
            <tr style="border-bottom:1px solid var(--border);">
              <td style="padding:7px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"><span class="engine-code">${escapeHtml(l.ma_thiet_bi || '')}</span></td>
              <td style="padding:7px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(l.ten_goi || '')}</td>
              <td style="padding:7px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(l.hang_muc || '')}</td>
              <td style="padding:7px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(l.ngay_thuc_hien || '')}</td>
              <td style="padding:7px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(l.nguoi_thuc_hien || '')}</td>
              <td style="padding:7px; word-break:break-word;">${escapeHtml(l.noi_dung || '')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  document.getElementById('close-stats-btn').addEventListener('click', closeModal);
  if (items.length) makeTableColumnsResizable(document.getElementById('stats-table'));
}

// Cho phép kéo cạnh phải mỗi cột (trừ cột cuối) để đổi độ rộng — dùng Pointer Events nên
// chạy được cả trên chuột lẫn cảm ứng (điện thoại/tablet).
function makeTableColumnsResizable(table) {
  if (!table) return;
  const cols = table.querySelectorAll('colgroup col');
  const ths = table.querySelectorAll('thead th');
  ths.forEach((th, i) => {
    if (i >= cols.length - 1) return; // cột cuối tự giãn theo phần còn lại, không cần núm kéo
    th.style.position = 'relative';
    const handle = document.createElement('div');
    handle.className = 'col-resize-handle';
    handle.style.cssText = 'position:absolute; top:0; right:-4px; width:9px; height:100%; cursor:col-resize; touch-action:none; z-index:2;';
...
async function openLogForm(log = null, presetEngine = null) {
  ...
  const materialLabel = (m) => m.name + (m.don_vi_tinh ? ' (' + m.don_vi_tinh + ')' : '');
  const sortedMaterials = [...state.materials].sort((a, b) => a.name.localeCompare(b.name, 'vi'));
  ...
  function renderMaterialRows() {
    ...
    wrap.innerHTML = materialRows.map((row, i) => {
      const selectedMaterial = row.material_id ? state.materials.find(m => String(m.id) === String(row.material_id)) : null;
      return `
      <div class="material-row" data-idx="${i}" style="display:flex; gap:6px; margin-bottom:6px; align-items:flex-start;">
        <div style="position:relative; flex:2;">
          <input type="text" class="material-combo-input" data-idx="${i}" autocomplete="off" placeholder="Gõ tên vật tư để tìm..."
            value="${selectedMaterial ? escapeAttr(materialLabel(selectedMaterial)) : ''}"
            style="width:100%; padding:8px 9px; border:1.5px solid var(--border); border-radius:7px;">
          <input type="hidden" class="material-combo-value" data-idx="${i}" value="${row.material_id || ''}">
          <div class="combo-results material-combo-results" data-idx="${i}" style="display:none;"></div>
        </div>
    ...
    wrap.querySelectorAll('.material-combo-input').forEach(inp => {
      const idx = Number(inp.dataset.idx);
      const hiddenVal = wrap.querySelector(`.material-combo-value[data-idx="${idx}"]`);
      const resultsBox = wrap.querySelector(`.material-combo-results[data-idx="${idx}"]`);

      function renderResults(query) {
        const q = query.trim().toLowerCase();
        const matches = !q ? sortedMaterials.slice(0, 40) : sortedMaterials.filter(m => materialLabel(m).toLowerCase().includes(q)).slice(0, 40);
        resultsBox.innerHTML = matches.length
          ? matches.map(m => `<div class="combo-result-item" data-mid="${m.id}">${escapeHtml(materialLabel(m))}</div>`).join('')
          : `<div class="combo-empty">Không tìm thấy vật tư nào</div>`;
        resultsBox.style.display = 'block';
        resultsBox.querySelectorAll('.combo-result-item').forEach(item => {
          item.addEventListener('mousedown', (ev) => {
            ev.preventDefault();
            const m = state.materials.find(x => String(x.id) === item.dataset.mid);
            inp.value = materialLabel(m);
            hiddenVal.value = m.id;
            materialRows[idx].material_id = m.id;
            resultsBox.style.display = 'none';
          });
        });
      }

      inp.addEventListener('focus', () => renderResults(inp.value));
      inp.addEventListener('input', () => { hiddenVal.value = ''; materialRows[idx].material_id = ''; renderResults(inp.value); });
      inp.addEventListener('blur', () => { setTimeout(() => { resultsBox.style.display = 'none'; }, 150); });
    });
...
let materialsYearFilter = ''; // rong = tat ca cac nam
...
async function renderMaterialsReport() {
  ...
  const allYears = [...new Set(usage.map(u => u.year))].sort();
  const years = materialsYearFilter ? [materialsYearFilter] : allYears;
  const byMaterial = new Map();
  state.materials.forEach(m => byMaterial.set(m.id, { name: m.name, unit: m.don_vi_tinh, byYear: {}, total: 0 }));
  usage.forEach(u => {
    if (materialsYearFilter && u.year !== materialsYearFilter) return;
  ...
  root.innerHTML = `
    <div class="card">
      ...
      <label style="font-size:12.5px; font-weight:600; color:var(--ink-dim); display:block; margin-bottom:10px;">Xem theo
        <select id="materials-year-filter" style="display:block; margin-top:4px; padding:9px 10px; border:1.5px solid var(--border); border-radius:7px; max-width:220px;">
          <option value="">Tất cả các năm</option>
          ${allYears.map(y => `<option value="${y}" ${materialsYearFilter === y ? 'selected' : ''}>Năm ${y}</option>`).join('')}
        </select>
      </label>
      <button class="btn btn-primary btn-block" id="open-material-manager-btn">⚙ Quản lý vật tư (${state.materials.length} loại)</button>
      <button class="btn btn-ghost btn-block" id="export-materials-btn" style="margin-top:8px;">⬇ Xuất Excel báo cáo tiêu hao vật tư${materialsYearFilter ? ' (năm ' + materialsYearFilter + ')' : ' (tất cả các năm)'}</button>
    </div>
    <div class="section-title">Tiêu hao theo năm</div>
    <div class="card" style="overflow-x:auto;">
      ${rows.length ? (() => {
        // Moi nam 1 mau nen rieng de doc nhanh theo cot
        const yearPalette = [
          { bg: '#e7f1fd', fg: '#1c5cad' },
          { bg: '#e6f6ef', fg: '#0f6e4e' },
          { bg: '#fdf0e3', fg: '#a34e08' },
        ...
        const colorOf = (i) => yearPalette[i % yearPalette.length];
        return `
        <table style="width:100%; border-collapse:collapse; font-size:14px;">
          <thead>
            <tr style="border-bottom:2px solid var(--border);">
              <th style="text-align:left; padding:9px 8px;">Vật tư</th>
              ${years.map((y, i) => `<th style="text-align:right; padding:9px 10px; background:${colorOf(i).bg}; color:${colorOf(i).fg}; font-size:15px;">${y}</th>`).join('')}
              <th style="text-align:right; padding:9px 10px; font-weight:800; font-size:15px; background:var(--steel); color:#fff;">Tổng</th>
            </tr>
          </thead>
          <tbody>
            ${rows.map(r => `
              <tr style="border-bottom:1px solid var(--border);">
                <td style="padding:9px 8px;">${escapeHtml(r.name)}${r.unit ? ` <span style="color:var(--ink-dim); font-size:12px;">(${escapeHtml(r.unit)})</span>` : ''}</td>
                ${years.map((y, i) => `<td style="text-align:right; padding:9px 10px; background:${colorOf(i).bg}; color:${colorOf(i).fg}; font-weight:${r.byYear[y] ? '700' : '400'}; font-size:${r.byYear[y] ? '15.5px' : '13px'};">${r.byYear[y] || '—'}</td>`).join('')}
                <td style="text-align:right; padding:9px 10px; font-weight:800; font-size:16px; background:var(--surface-2);">${r.total || '—'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  document.getElementById('materials-year-filter').addEventListener('change', (e) => {
    materialsYearFilter = e.target.value;
  ...
  document.getElementById('export-materials-btn').addEventListener('click', () => {
    window.location.href = '/api/materials/export' + (materialsYearFilter ? '?year=' + encodeURIComponent(materialsYearFilter) : '');
  });
}
// Trạng thái + nhật ký đồng bộ OneDrive, hiện trong tab Dữ liệu (mục "Đồng bộ OneDrive")
async function renderOnedriveStatus() {
  const line = document.getElementById('onedrive-status-line');
  const logBox = document.getElementById('onedrive-log');
  if (!line || !logBox) return;
  line.textContent = 'Đang tải trạng thái...';
  try {
    const st = await api('/data/onedrive-status');
    if (!st.enabled) {
      line.innerHTML = '⚪ Chưa bật đồng bộ OneDrive — bật "onedrive_backup_enabled" trong tab Configuration của add-on.';
    } else if (!st.ready) {
      line.innerHTML = '🟠 Đã bật nhưng chưa cấu hình xong (thiếu onedrive_token / onedrive_remote_path / onedrive_drive_id) — xem nhật ký bên dưới.';
    } else {
      line.innerHTML = `🟢 Đang hoạt động — đích: <b>${escapeHtml(st.remotePath)}</b>`;
    }
    logBox.innerHTML = (st.log && st.log.length)
      ? st.log.map(l => `<div>${escapeHtml(l)}</div>`).join('')
      : '<div>Chưa có nhật ký nào.</div>';
  } catch (err) {
    line.textContent = 'Không tải được trạng thái: ' + err.message;
  }
}
...
async function renderData() {
  ...
  root.innerHTML = `
    ...
    <div class="card">
      ...
      <div style="margin-top:10px;">
        ...
        <label style="font-size:13px; display:block; margin-bottom:4px;"><input type="radio" name="import-target" value="maintenance_wide"> Lịch sử bảo trì - dạng nhiều hạng mục theo cột (vd file "Đợt kiểm tra")</label>
        <label style="font-size:13px; display:block;"><input type="radio" name="import-target" value="materials"> Danh sách vật tư</label>
      </div>
    ...
    <div class="section-title">Đồng bộ OneDrive</div>
    <div class="card">
      <p style="font-size:13px; color:var(--ink-dim); margin-top:0;">
        Ngay sau khi xuất file (tự động hằng tuần hoặc bấm "Xuất Excel ngay" ở trên), hệ thống tự đẩy file đó
        lên thư mục OneDrive đã cấu hình — qua Internet, không cần máy tính nào bật hay cùng mạng LAN.
        Bật/điền "onedrive_backup_enabled", "onedrive_token", "onedrive_remote_path"... trong tab
        <b>Configuration</b> của add-on này (dùng lại đúng token/drive_id đã cấu hình cho app Chấm công cũng
        được — chỉ cần đổi "onedrive_remote_path" sang thư mục riêng cho Bảo trì).
      </p>
      <div id="onedrive-status-line" style="font-size:13px; margin-bottom:10px;">Đang tải trạng thái...</div>
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        <button class="btn btn-primary" id="onedrive-sync-now-btn" style="flex:1; min-width:170px;">☁ Đồng bộ OneDrive ngay</button>
        <button class="btn btn-ghost" id="onedrive-refresh-btn" style="flex:1; min-width:170px;">⟳ Tải lại nhật ký</button>
      </div>
      <div id="onedrive-log" style="margin-top:10px; max-height:160px; overflow-y:auto; font-size:12px; color:var(--ink-dim); font-family:monospace; line-height:1.6;"></div>
    </div>
  ...
  document.getElementById('onedrive-refresh-btn').addEventListener('click', () => renderOnedriveStatus());
  document.getElementById('onedrive-sync-now-btn').addEventListener('click', async () => {
    try {
      const r = await api('/data/onedrive-sync-now', { method: 'POST' });
      toast(`Đã gửi lệnh đồng bộ cho ${r.count} file`);
      setTimeout(renderOnedriveStatus, 3000);
    } catch (err) { toast(err.message); }
  });
...
function openImportMappingModal(previewData, target) {
  let targetFields = target === 'engines' ? [...previewData.targetFieldsEngine]
    : target === 'materials' ? [...previewData.targetFieldsMaterials]
    : [...previewData.targetFieldsMaintenance];
  let currentMapping = {};
  ...
  function render() {
    ...
    if (target === 'maintenance_wide') {
      ...
      bodyHtml = `
        ...
        ` : ''}
        ${target === 'engines' || target === 'materials' ? `
    ...
    if (target === 'maintenance_wide') {
      ...
      document.querySelectorAll('[data-create-field]').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          const isMaterial = target === 'materials';
          const endpoint = isMaterial ? '/material-fields' : '/engine-fields';
          try {
            const created = await api(endpoint, { method: 'POST', body: JSON.stringify({ label: headerName }) });
            targetFields.push({ key: created.field_key, label: created.label });
            currentMapping[created.field_key] = headerName;
            if (isMaterial) await loadMaterialFields(true);
            else await loadEngineFields(true);
