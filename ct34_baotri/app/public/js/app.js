const state = { user: null, view: 'dashboard', engines: [], engineMap: {}, engineFields: [], logMap: {} };
...
async function loadHangMucGoiY() {
  const { items } = await api('/maintenance/hang-muc-goi-y');
  hangMucGoiY = items;
  return items;
}
...
async function renderMaintenance() {
  ...
  root.innerHTML = `
    ...
    <div class="filter-chips" id="hangmuc-chips">
      ...
      ${hangMucGoiY.slice(0, 10).map(h => chip(h, h)).join('')}
    </div>
...
async function openLogForm(log = null, presetEngine = null) {
  ...
  if (!hangMucGoiY.length) await loadHangMucGoiY();
  ...
  const html = `
    ...
    <form id="log-form" class="form-grid">
      ...
      <div class="full"><label>Hạng mục / loại công việc
        <input type="text" name="hang_muc" list="hang-muc-datalist" value="${escapeHtml(log ? (log.hang_muc || '') : '')}" placeholder="vd: Vệ sinh, bảo dưỡng...">
        <datalist id="hang-muc-datalist">
          ${hangMucGoiY.map(h => `<option value="${escapeAttr(h)}">`).join('')}
        </datalist>
      </label></div>
...
function openImportMappingModal(previewData, target) {
  ...
  const guessMaCol = () => previewData.headers.find(h => /mã|ma\s*thiết\s*bị|cụm\s*tb|cum\s*tb/i.test(h)) || previewData.headers[0] || '';
  ...
  function render() {
    ...
    if (target === 'maintenance_wide') {
      ...
      if (!previewData.categoryPairs || !previewData.categoryPairs.length) {
        ...
        bodyHtml = `
          <label style="font-size:13px; font-weight:600; color:var(--ink-dim); display:block; margin-bottom:12px;">Cột mã thiết bị
            <select id="ma-col-select" style="width:100%; margin-top:4px; padding:9px 10px; border:1.5px solid var(--border); border-radius:7px;">
    ...
    document.getElementById('confirm-import-btn').addEventListener('click', async () => {
      ...
      try {
        const result = await api('/data/commit', {
          method: 'POST',
          body: JSON.stringify({ uploadId: previewData.uploadId, target, sheetName: previewData.currentSheet, mapping }),
        });
        closeModal();
