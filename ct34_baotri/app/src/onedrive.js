...
function syncFileToOnedrive(localFilePath) {
  ...
  // -vv de ep rclone in chi tiet (mac dinh rclone hay im lang, khong ghi gi vao stderr ca khi
  // loi xay ra o buoc chuan bi/xac thuc thay vi loi transfer file thong thuong).
  const args = ['--config', RCLONE_CONF_PATH, 'copy', localFilePath, `onedrive:${ONEDRIVE_REMOTE_PATH}`, '--onedrive-no-versions', '-vv'];
  execFile('rclone', args, { timeout: 120000, maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
    if (err) {
      // Ghi ca stdout lan stderr, cong voi ma thoat/signal - phong truong hop rclone khong in gi
      // vao stderr (vi du loi xay ra truoc khi vao vong log binh thuong).
      const parts = [];
      const errTxt = (stderr && stderr.toString().trim()) || '';
      const outTxt = (stdout && stdout.toString().trim()) || '';
      if (errTxt) parts.push('stderr: ' + errTxt);
      if (outTxt) parts.push('stdout: ' + outTxt);
      parts.push(`exitCode=${err.code !== undefined ? err.code : '?'}${err.signal ? ' signal=' + err.signal : ''}`);
      if (!errTxt && !outTxt) parts.push('msg: ' + err.message);
      const detail = parts.join(' || ').replace(/\s+/g, ' ').trim().slice(0, 800);
