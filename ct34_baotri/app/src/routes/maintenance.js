...
router.get('/', (req, res) => {
  ...
  if (q) {
    where.push('(e.ma_thiet_bi LIKE ? OR e.data_json LIKE ? OR t.mo_ta LIKE ?)');
    const like = `%${q}%`;
  ...
  const items = rows.map(r => {
    ...
    return { ...rest, ten_goi: displayLabel(engine_data_json), trang_thai_hien_thi: recomputeStatus(r) };
  });
  res.json({ items });
});
...
router.get('/:id', (req, res) => {
  ...
  res.json({ ...rest, ten_goi: displayLabel(engine_data_json), logs });
});
