...
const { getMaterialFields } = require('../materialFields');
const { listSheetNames, readSheetSafely } = require('../safeExcelReader');
const { syncFileToOnedrive, syncFilesSequentially, getOnedriveStatus } = require('../onedrive');
...
function detectDataStartRow(sheet, mergeIndex) {
  ...
  const colCheckLimit = Math.min(sheet.columnCount, 8);
  for (let r = 2; r <= scanLimit; r++) {
    const a = cellNumber(getCellValue(sheet, mergeIndex, r, 1));
    if (a === null) continue;
    // Xac nhan day thuc su la dong du lieu: co it nhat 1 o noi dung chu o vai cot dau
    // (khong chi dua vao dong ke tiep co STT hay khong, vi STT co the bi bo sot ram rac).
    let hasOtherContent = false;
    for (let c = 2; c <= colCheckLimit; c++) {
      if (cellText(getCellValue(sheet, mergeIndex, r, c))) { hasOtherContent = true; break; }
    }
    if (hasOtherContent) return r;
  }
  return null;
}
...
function buildPreviewPayload(sheet, mergeIndex) {
  ...
  const materialFields = getMaterialFields().map(f => ({ key: f.field_key, label: f.label }));
  ...
  return {
    ...
    targetFieldsMaterials: [{ key: 'name', label: 'Tên vật tư (bắt buộc, dùng làm định danh)' }, ...materialFields],
    categoryPairs,
  };
}
...
router.post('/commit', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (target === 'engines') {
      ...
      const materialFieldKeys = getMaterialFields().map(f => f.field_key);
      const nameCol = mapping['name'] ? headerNames[mapping['name']] : null;
      const maxOrder0 = db.prepare('SELECT COALESCE(MAX(sort_order), -1) m FROM materials').get().m;
      let nextOrder = maxOrder0 + 1;
      const upsert = db.prepare(`
        INSERT INTO materials (name, data_json, sort_order) VALUES (?, ?, ?)
        ON CONFLICT(name) DO UPDATE SET data_json = excluded.data_json
      `);

      for (let i = dataStartRow; i <= sheet.rowCount; i++) {
        const name = nameCol ? cellText(getCellValue(sheet, mergeIndex, i, nameCol)) : '';
        if (!name) { skipped++; continue; }

        const data = {};
        for (const fk of materialFieldKeys) {
          const excelCol = mapping[fk];
          const colNum = excelCol ? headerNames[excelCol] : null;
          const v = colNum ? cellText(getCellValue(sheet, mergeIndex, i, colNum)) : '';
          if (v) data[fk] = v;
        ...
        const before = db.prepare('SELECT id FROM materials WHERE name = ?').get(name);
        upsert.run(name, JSON.stringify(data), nextOrder);
        if (before) updated++; else { inserted++; nextOrder++; }
      }
    } else if (target === 'maintenance') {
      const fixedValues = req.body.fixedValues || {};
      for (let i = dataStartRow; i <= sheet.rowCount; i++) {
        const rec = {};
        let hasColumnData = false; // co it nhat 1 truong lay tu cot that trong file (khac rong), khong tinh gia tri co dinh
        for (const f of MAINT_TARGET_FIELDS) {
          const excelCol = mapping[f.key];
          const colNum = excelCol ? headerNames[excelCol] : null;
          if (colNum) {
            const v = cellText(getCellValue(sheet, mergeIndex, i, colNum)) || null;
            rec[f.key] = v;
            if (v && f.key !== 'ma_thiet_bi') hasColumnData = true;
          } else if (fixedValues[f.key]) {
            rec[f.key] = String(fixedValues[f.key]).trim() || null; // gia tri co dinh ap dung cho moi dong
          } else {
            rec[f.key] = null;
          }
        }
        if (!rec.ma_thiet_bi) { skipped++; continue; }
        if (!hasColumnData) { skipped++; continue; } // dong nay khong co du lieu that su, chi co gia tri co dinh -> bo qua
...
// Trạng thái đồng bộ OneDrive (đã bật chưa, đã cấu hình xong chưa, nhật ký gần đây)
router.get('/onedrive-status', requireAdmin, (req, res) => {
  res.json(getOnedriveStatus());
});

// Đồng bộ thủ công: đẩy lại toàn bộ file .xlsx đang có trong thư mục xuất lên OneDrive ngay.
// Chạy LẦN LƯỢT từng file (không bắn song song) để tránh bị Microsoft trả "serviceNotAvailable"
// khi nhiều tiến trình rclone cùng gọi tới cùng 1 ổ đĩa OneDrive một lúc.
router.post('/onedrive-sync-now', requireAdmin, (req, res) => {
  const status = getOnedriveStatus();
  if (!status.ready) {
    return res.status(400).json({ error: 'Chưa bật/cấu hình xong đồng bộ OneDrive (xem tab Configuration của add-on).' });
  }
  try {
    const dir = process.env.EXPORT_DIR || '/share/baotri_exports';
    fs.mkdirSync(dir, { recursive: true });
    const files = fs.readdirSync(dir).filter(f => f.endsWith('.xlsx'));
    syncFilesSequentially(files.map(f => path.join(dir, f))); // khong await - chay nen, ket qua xem qua nhat ky
